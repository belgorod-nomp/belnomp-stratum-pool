...
var JobManager = module.exports = function JobManager(options) {
    ...
    this.processShare = function (jobId, previousDifficulty, difficulty, extraNonce1, extraNonce2, nTime, nonce, ipAddress, port, workerName, versionBit, asicBoost, versionMask, origVersion) {
        ...
        if ((asicBoost === true) && (versionBit !== undefined)) {
	    // versionBit & versionmask are string, origVersion is uint.
	    var vBit = parseInt("0x"+versionBit);
	    var vMask= parseInt("0x"+versionMask);
            if ((vBit & ~vMask) !== 0) {
                return shareError([24, 'versionBit incorrect']);
            }
            version = (origVersion & ~vMask) | (vBit & vMask);            
        } else {
            version = origVersion;
        }
        ...
        var headerBuffer = job.serializeHeader(merkleRoot, nTime, nonce, version);
